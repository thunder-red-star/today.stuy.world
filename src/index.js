...
setInterval(() => {
    ...
    io.emit('heartbeat', {
        ...
        currentClassTimeSince: new Date().getTime() - ThunderUtils.time.epochToCurrent(currentClass.startTime).getTime(),
        // Do the same for the end time but subtract in the opposite direction.
        currentClassTimeUntil: ThunderUtils.time.epochToCurrent(currentClass.endTime).getTime() - new Date().getTime(),
    });
}, 1000);
